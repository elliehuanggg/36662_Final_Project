...
**FINISH justification, maybe about parametric vs. nonparametrics??**
...
the nature of the relationship between the predictors in your model and the predictions themselves. Based on the feature importance from Figure \@ref(fig:fig-feat-import), the features with the strongest feature importance were `visible_carrier_delay_rate`, `sched_dep_hour`, `sched_arr_hour` and `route_delay_rate_smooth`. This shows that the timing of the flights and the historical delays of the carrier and routes make strong predictors for delays. Based on this, it's possible that flights with specific times and on specifics carriers or routes are more easily predicted.
...
If this transitioned into a regression problem, where we made continuous predictions about the minutes delayed, we would shift from the AUC loss function to the mean absolute error as our loss function because it's a loss function suitable to regression and, since the delay times can be strongly right-skewed, it will be more robust to outliers (as compared to the traditional mean squared error). In addition, we would use regression machine-learning methods with varying complexity and again use an ensemble predictor using both a naive average and a weighted average. To start, we would use linear regression with and without LASSO to begin with a simple model. Then, we would transition to more complex methods, such as XGBoost and random forest (again, the naive random forest and balanced random forest).

## Trade-Off of a Fixed Cost

Assume that we cared about a trade-off of a fixed cost C for missing a flight and an hourly cost r of waiting for a delayed flight. We would transition to a regression problem, similar to above (i.e. where we made continuous predictions about the minutes delayed), and we would use the regression methods highlighted above. However, instead of mean absolute error, we would use the following loss function:
...
We are transitioning to squared error for underpredicting delays because we no longer want a loss function that is robust to outliers. In this case, waiting for a delayed flight has loss proportional to the length of the delay (i.e. *r*), so we want strong outliers to be influential since they represent a huge loss contextually. In addition, we are implementing weighted MSE because there is a proportional loss for predicting 0 when there is any delay (i.e. $r$) and a fixed-rate loss for predicting a delay when the flight is on-time or early (i.e. $C$).